...
export function Features() {
    return (
        ...
        >
            {/* Background glow */}
            <div className="pointer-events-none absolute top-1/3 right-0 h-[520px] w-[520px] rounded-full bg-purple-300/20 dark:bg-purple-500/30 blur-[140px]" />
                ...
                <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                    {features.map((feature, index) => {
                        ...
                        return (
                            <Card
                                key={index}
                                className="group relative overflow-hidden rounded-3xl border-0 bg-white/70 dark:bg-gray-900/70 backdrop-blur-xl shadow-[0_20px_50px_rgba(140,120,255,0.18)] dark:shadow-[0_20px_50px_rgba(140,80,255,0.5)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_30px_70px_rgba(140,120,255,0.25)] dark:hover:shadow-[0_30px_70px_rgba(140,80,255,0.7)] animate-in fade-in slide-in-from-bottom"
                                style={{ animationDelay: `${index * 100}ms` }}
                            >
                                {/* Hover gradient overlay */}
                                <div className="pointer-events-none absolute inset-0 rounded-3xl bg-gradient-to-br from-purple-100/0 to-blue-100/0 dark:from-purple-500/0 dark:to-blue-500/0 transition-all duration-500 group-hover:from-purple-100/50 group-hover:to-blue-100/50 dark:group-hover:from-purple-500/20 dark:group-hover:to-blue-500/20" />

                                <CardHeader className="relative z-10 space-y-5 pt-8">
                                    {/* Icon badge */}
                                    <div className="flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-purple-200 to-blue-300 dark:from-purple-600 dark:to-blue-600 shadow-lg dark:shadow-purple-500/50 transition-all duration-300 group-hover:scale-110 group-hover:shadow-purple-300/50 dark:group-hover:shadow-purple-500/80">
                                        <Icon className="h-10 w-10 text-purple-700 dark:text-white" />
                                    </div>

                                    <CardTitle className="text-xl font-semibold text-gray-900 dark:text-white transition-colors duration-300 group-hover:text-purple-700 dark:group-hover:text-purple-400 sm:text-2xl">
                                        {feature.title}
                                    </CardTitle>
                                </CardHeader>

                                <CardContent className="relative z-10 pb-8">
                                    <CardDescription className="text-base font-light leading-relaxed text-gray-600 dark:text-gray-300 sm:text-lg">
                                        {feature.description}
                                    </CardDescription>
                                </CardContent>
                            </Card>
